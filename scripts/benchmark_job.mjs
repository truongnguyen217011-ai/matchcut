...
const settings = { ...(state.profiles?.[state.activeProfile] || {}), fastRender: true };
...
form.append("selectionMode", settings.mediaSelectionMode || "shuffle");
...
console.log(JSON.stringify({ id: job.id, footage: folderData.files.length, startedAt: new Date().toISOString() }));
