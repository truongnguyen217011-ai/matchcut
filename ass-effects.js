export function applyAssTextEffect(content, placement, scene, settings, accent) {
  ...
  if (settings.textEffect === "karaoke") {
    ...
    text = `${placement}${words.map((word) => word === "\\N" || /^\s+$/.test(word) ? word : `{\\k${centis}}${word}`).join("")}`;
  }
  if (settings.textEffect === "typewriter") {
    ...
    text = `${placement}${tokens.map((token) => token === "\\N" ? token : [...token].map((character) => `{\\k${centis}}${character}`).join("")).join("")}`;
  }
